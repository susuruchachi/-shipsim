...
//     張り出しを表現できる核心。
// ─────────────────────────────────────────
//   extentSegs : 輪郭の前後端（正規化の基準）を決めるのに使う線分群。
//                外板本体だけを渡す。スクリュー軸や舵のように船体より後方へ
//                突き出た付属物まで前後端の決定に混ぜると、輪郭が付属物の先まで
//                引き伸ばされ、船尾が細長いくさび形に化けてしまう（この表現は
//                1本の閉じた輪郭なので、離れた細い突起を別パーツとして持てない）。
//                幅のサンプリングには付属物を含む全線分(segsFlat)を使うので、
